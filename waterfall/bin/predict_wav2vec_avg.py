...
def predict(data_dir,
    ...
    if len(all_results) == 1:
        ...
        for i in range(len(all_results[0])):
            log_probs = torch.stack([all_results[j][i][0] for j in range(len(all_results))], dim=0)
            log_probs = torch.logsumexp(log_probs, dim=0) - np.log(len(all_results))
            results.append((log_probs, all_results[0][i][1], all_results[0][i][2], all_results[0][i][3], all_results[0][i][4]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, help='the path of the models, separated by comma')
