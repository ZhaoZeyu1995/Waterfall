...
def align(data_dir, lang_dir, model_dir, output_dir, jid, gpus=0, batch_size=1):
    """
    ...
    with WriteHelper(
        "ark,scp:%s,%s"
        % (
            os.path.join(os.getcwd(), output_dir, "align.%d.ark" % (jid)),
            os.path.join(os.getcwd(), output_dir, "align.%d.scp" % (jid)),
        )
    ) as writer:
        for item in results:
