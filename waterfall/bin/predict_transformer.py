...
def predict(data_dir, lang_dir, model_dir, output_dir, jid, gpus=1, batch_size=1):
    ...
    model = transformer.TransformerModel.load_from_checkpoint(model_dir)
    ...
    tic = time.time()
    data_gen = DataLoader(
        Dataset(data_dir, lang_dir, load_feats=True),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Get the outputs (posterior probabilities) from a transformer model."
    )
