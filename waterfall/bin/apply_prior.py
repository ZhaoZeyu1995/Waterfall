...
def main(args):
    ...
    write_ark = os.path.join(os.getcwd(), args.output_dir,
                             'output.%d.ark' % (args.jid))
    write_scp = os.path.join(os.getcwd(), args.output_dir,
                             'output.%d.scp' % (args.jid))

    with kaldiio.ReadHelper('scp:%s' % (read_scp)) as reader, kaldiio.WriteHelper('ark,scp:%s,%s' % (write_ark, write_scp)) as writer:
