...
def main(args):
    ...
    if cfg["loss"] in ["ctc_k2", "k2"]:
        train_data = Dataset(args.train_set, args.lang_dir, token_type="phones")
        dev_data = Dataset(args.dev_set, args.lang_dir, token_type="phones")
    else:
    ...
    model_checkpoint = pl.callbacks.ModelCheckpoint(
        monitor="valid_loss",
        save_top_k=1 if "save_top_k" not in cfg.keys() else cfg["save_top_k"],
        every_n_epochs=1,
    ...
    logger = pl.loggers.WandbLogger(project=args.name, save_dir="exp/%s" % (args.name))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_weights_only",
        help="Whether or not load weights only from checkpoint.",
        type=bool,
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="The number of batches for gradient accumulation.",
        type=int,
        default=1,
    )
