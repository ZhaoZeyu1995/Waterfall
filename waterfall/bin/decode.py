...
def decode(
    ...
    dec = build_ctcdecoder(labels=labels, kenlm_model_path=lm, beta=beta, alpha=alpha)

    result_path = os.path.join(
        os.path.dirname(predict_path),
        "alpha_%.3f_beta_%d_beamsize_%d_beamprune_%d"
        % (alpha, beta, beam_size, beam_prune),
    )
    os.makedirs(result_path, exist_ok=True)

    with open(
        os.path.join(os.getcwd(), result_path, "hyp.wrd.trn.%d" % (jid)), "w"
    ) as x:
        xc = ""
        with ReadHelper(
            "scp:%s"
            % (os.path.join(os.getcwd(), predict_path, "output.%d.scp" % (jid)))
        ) as reader:
            for uttid, data in reader:
                result = dec.decode(
                    data,
                    beam_width=beam_size,
                    token_min_logp=prune,
                    beam_prune_logp=beam_prune,
                )
                xc += "%s (%s-%s)\n" % (result, utt2spk[uttid], uttid)
        x.write(xc)
