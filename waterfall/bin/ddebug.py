...
class DebuggerModel(models.Wav2VecFineTuningDiverse):
    ...
    def division(self, batch, batch_idx=None, optimizer_idx=None):
        if self.cfg["loss"] == "ctc":
            ...
            decoding_graph = k2.compose(topo, trans_fsa, treat_epsilons_specially=False)
...
def main(args):
    ...
    train_data = Dataset(args.train_set, args.lang_dir)
    dev_data = Dataset(args.dev_set, args.lang_dir)
    ...
    if cfg["early_stopping"]:
        callbacks.append(
            pl.callbacks.EarlyStopping(
                monitor="valid_loss", mode="min", patience=cfg["patience"], verbose=True
            )
        )
    ...
    if "auto_eta_scheduler" in cfg.keys():
        if cfg["auto_eta_scheduler"]:
            callbacks.append(
                eta_scheduler.AutoEtaScheduler(
                    "valid_loss",
    ...
    trainer = pl.Trainer(
        ...
        logger=pl.loggers.TensorBoardLogger("exp", name=args.name),
        callbacks=callbacks,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_set", help="Training set directory.", type=str)
    ...
    parser.add_argument(
        "--name",
        help="Experiment name. Models will be stored in exp/$name/version*",
        type=str,
        default="ctc",
    )
    parser.add_argument(
