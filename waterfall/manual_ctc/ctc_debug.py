...
class CTC(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        print('torch.exp(grad_num_token)[0, 0, :]',
              torch.exp(grad_num_token)[0, 0, :])
        print('torch.exp(grad_num_token)[0, 1, :]',
              torch.exp(grad_num_token)[0, 1, :])
        print('torch.exp(grad_num_token)[0, 5, :]',
              torch.exp(grad_num_token)[0, 5, :])

        expanded_prob_transform = torch.exp(transform).unsqueeze(
            1).expand(B, T, S, C).permute(0, 1, 3, 2).reshape(B*T, C, S)
        prob_grad_num_token = torch.exp(
            grad_num_token).unsqueeze(-1).reshape(B*T, S, 1)

        print('expanded_prob_transform[0, 0, :]',
              expanded_prob_transform[0, 0, :])
        print('expanded_prob_transform[0, 3, :]',
              expanded_prob_transform[0, 3, :])

        print('prob_grad_num_token[0, :, 0]', prob_grad_num_token[0, :, 0])
        print('prob_grad_num_token[20, :, 0]', prob_grad_num_token[20, :, 0])

        result = torch.matmul(expanded_prob_transform,
                              prob_grad_num_token).reshape(B, T, C)
        ...
        result2 = torch.exp(log_probs + log_probs)-torch.exp(log_probs +
                                                             grad_num) / torch.exp(log_probs) - torch.exp(log_probs + log_probs)
