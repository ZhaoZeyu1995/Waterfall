...
class CTC_Softmax(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        for t in range(1, T+1):
            ...
            log_beta[:, t, :] = 1/eta * torch.logsumexp(eta * (trans.permute(
                0, 2, 1) + log_beta[:, t-1, :].unsqueeze(1)),
...
def ctc_softmax(log_probs: torch.tensor,
                input_lengths: torch.tensor,
                trans: torch.tensor,
                transform: torch.tensor,
                trans_lengths: torch.tensor,
                eta=1.0,
                reduction='sum'):
    return CTC_Softmax.apply(log_probs,
                             input_lengths,
                             trans,
                             trans_lengths,
                             transform,
