...
class Lang(object):
    ...
    def __init__(self, lang_dir, load_topo=False, load_lexicon=False):
        """
    ...
    def compile_denominator_graph(self):
        ...
        self.den_graph = k2.arc_sort(
            k2.Fsa.from_openfst(os.popen(cmd).read(), acceptor=False)
        )
    ...
    def compile_training_graph_bak(self, word_ids_list, device):
        ...
        training_graph = k2.remove_epsilon(
            k2.compose(
                self.topo, trans_fsa_with_self_loop, treat_epsilons_specially=False
            )
        )
    ...
    def __init__(
        ...
        if self.min_frames is not None:
            num_short_utt = len(
                [
                    ...
                    if self.utt2num_frames[uttid] < self.min_frames
                ]
            )
            logging.info(
                f"Filtering utterances with less than {self.min_frames} frames, {num_short_utt} utterances are removed"
            )
            self.uttids = [
                uttid
                for uttid in self.uttids
        ...
        if self.sort is not None:
            ...
            if self.sort == "ascending":
                logging.info(
                    "Sorting utterances by ascending order of number of frames"
                )
                self.uttids = sorted(self.uttids, key=lambda x: self.utt2num_frames[x])
            else:
                logging.info(
                    "Sorting utterances by descending order of number of frames"
                )
                self.uttids = sorted(
                    self.uttids, key=lambda x: self.utt2num_frames[x], reverse=True
                )
    ...
    def __getitem__(self, idx):
        ...
        if self.ratio_th:
            if int(num_frame / self.ratio_th) < len(pids):
                logging.warn(
                    "The num_frame %d is not enough for the utterance %s, we will take the last utterance in the dataset"
                    % (num_frame, uttid)
                )
...
def collate_fn(list_of_samples):
    ...
    samples_collated = {
        ...
        "targets_ctc": torch.nn.utils.rnn.pad_sequence(
            batch_targets_ctc, batch_first=True
        ),
        "names": batch_names,
        "spks": batch_spks,
        "durs": batch_durs,
    ...
    if "wav" in list_of_samples[0].keys():
        ...
        samples_collated["wav_lens"] = torch.tensor(batch_wav_lengths, dtype=torch.long)
...
def collate_fn_sorted(list_of_samples):
    ...
    if "wav" in list_of_samples[0].keys():
        ...
        batch_target_lengths = [
            x
            for _, x in sorted(
                zip(batch_lengths, batch_target_lengths),
                key=lambda x: x[0],
                reverse=True,
            )
        ]
        ...
        return {
            "wavs": torch.nn.utils.rnn.pad_sequence(batch_wav, batch_first=True),
    ...
    if "feats" in list_of_samples[0].keys():
        ...
        batch_targets_ctc = [
            x
            for _, x in sorted(
                zip(batch_feats_lens, batch_targets_ctc),
                key=lambda x: x[0],
                reverse=True,
            )
        ]
        batch_target_lengths = [
            x
            for _, x in sorted(
                zip(batch_feats_lens, batch_target_lengths),
                key=lambda x: x[0],
                reverse=True,
            )
        ]
        ...
        batch_num_frames = [
            x
            for _, x in sorted(
                zip(batch_feats_lens, batch_num_frames),
                key=lambda x: x[0],
                reverse=True,
            )
        ]
        ...
        return {
            "feats": torch.nn.utils.rnn.pad_sequence(batch_feats, batch_first=True),
