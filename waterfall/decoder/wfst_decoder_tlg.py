...
class WFSTDecoder:
    ...
    def init_decoding(self):
        ...
        self.cur_toks[(start_state_t, start_state_lg)] = Token(dummy_arc, 0.0, None)
                ...
                if arc.ilabel == 0:
                    ...
                    if new_tok.cost > cutoff:
                        ...
                        if (state_t, arc.nextstate) in self.cur_toks.keys():
                            ...
                            if (
                                self.cur_toks[(state_t, arc.nextstate)].cost
                                > new_tok.cost
                            ):
                                # print('Nonemmiting Updating token for ', (state_t, arc.nextstate))
                                delete_token(self.cur_toks[(state_t, arc.nextstate)])
