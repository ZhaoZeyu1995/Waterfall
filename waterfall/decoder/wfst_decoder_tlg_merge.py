...
class WFSTDecoder:
        ...
        for (state_t, state_lg), toks in tokens.items():
            for tok in toks:
                if (state_lg, toks.arc) not in merged_tokens2cost:
                    ...
                    merged_tokens2cost[(state_lg, tok.arc)] = -np.logaddexp(-merged_tokens2cost[(state_lg, tok.arc)], -tok.cost)
        # Prune
    ...
    def init_decoding(self):
        ...
        self.cur_state_lg_arc_lg[(start_state_lg, dummy_arc)] = {Token(dummy_arc, 0.0, start_state_t, None)}
        ...
        if best_token is not None:  # Process the best token first, and hopefully find a proper next_weight_cutoff
            ...
            for arc_t in self.t_fst.arcs(best_state[0]):
                ...
                if arc_t.olabel != 0:
                    ...
                    new_weight = best_token.cost + ac_cost # we don't have to transit in LG.
                    if new_weight + adaptive_beam < next_weight_cutoff:  # make next_weight_cutoff tighter
                        next_weight_cutoff = new_weight + adaptive_beam
                ...
                for arc in self.lg_fst.arcs(state_lg):
                    if arc.ilabel == 0:
                        ...
                        if (arc.nextstate, arc) in self.cur_toks.keys():
                            ...
                            self.cur_toks2cost[(arc.nextstate, arc)] = -np.logaddexp(-self.cur_toks2cost[(arc.nextstate, arc)], -new_tok.cost)
    ...
    def get_best_path(self):
        ...
        if not is_final:
            ...
            for state, tok in self.cur_toks.items():
                ...
                this_cost = tok.cost + float(self.lg_fst.final(state[1])) # We only take the weight from LG.
