...
class WFSTDecoder:
    ...
    def init_decoding(self):
        ...
        )  # (state_lg, ilabel, olabel) -> Token(arc_lg, ac_cost, {t_states}, prev)
    ...
    def process_emitting(self):
        ...
        if (
            ...
            for state_t in best_token.t_states:
                for arc_t in self.t_fst.arcs(state_t):
                    ...
                    ac_cost = self.log_likelihood_scaled[frame, int(arc_t.ilabel) - 1]
                    if arc_t.olabel != 0:
                        for arc_lg in self.lg_fst.arcs(best_state):
                            ...
                            if arc_lg.ilabel == arc_t.olabel:
                                ...
                                if arc_lg.nextstate not in accum_cost:
                                    ...
                                    accum_cost[arc_lg.nextstate] = -np.logaddexp(
                                        -accum_cost[arc_lg.nextstate], -new_weight
                                    )
                        ...
                        if best_state not in accum_cost:
                            ...
                            accum_cost[best_state] = -np.logaddexp(
                                -accum_cost[best_state], -new_weight
                            )
        for _, cost in accum_cost.items():
            if (
                cost + adaptive_beam < next_weight_cutoff
        ...
        accum_toks = (
            dict()
        )  # (state_lg, ilabel, olabel) -> Token(arc_lg, ac_cost_accum, {state_t}, prev)
            ...
            if tok.cost < weight_cutoff:
                ...
                for state_t in tok.t_states:
                    for arc_t in self.t_fst.arcs(state_t):
                        ...
                        if (
                            arc_t.olabel == 0
                            and (state_lg, None, tok) not in pruned_toks
                        ):  # we only update T state in this case
                            ...
                            if new_weight < next_weight_cutoff:
                                ...
                                if state_lg in self.cur_toks:  # only update T state
                                    ...
                                    self.cur_toks[state_lg].update_t_states(
                                        arc_t.nextstate
                                    )
                                else:
                                    dummy_arc = LatticeArc(0, 0, 0.0, state_lg)
                                    new_tok = Token(
                                        dummy_arc, ac_cost, {arc_t.nextstate}, tok
                                    )
                            ...
                            for arc_lg in self.lg_fst.arcs(state_lg):
                                ...
                                if (
                                    arc_t.olabel == arc_lg.ilabel
                                    and (arc_lg.nextstate, arc_lg, tok)
                                    not in pruned_toks
                                ):
                                    ...
                                    new_weight = (
                                        float(arc_lg.weight) + tok.cost + ac_cost
                                    )
                                    if new_weight < next_weight_cutoff:
                                        ...
                                        if (
                                            ...
                                            accum_toks[
                                                (arc_lg.nextstate, arc_lg, tok)
                                            ].update_ac_cost(arc_lg)
                                            accum_toks[
                                                (arc_lg.nextstate, arc_lg, tok)
                                            ].update_t_states(arc_t.nextstate)
                                            ...
                                            accum_toks[
                                                ...
                                                arc_lg, ac_cost, {arc_t.nextstate}, tok
                                            )
                                    else:
                                        pruned_toks.append(
                                            (arc_lg.nextstate, arc_lg, tok)
