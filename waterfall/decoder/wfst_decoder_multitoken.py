...
class Token:
    ...
    def __init__(self, arc, acoustic_cost, prefix=(), prev_tok=None):
...
class WFSTDecoder:
    ...
    def decode(self, log_likelihood: np.array):
        ...
        while self.num_frames_decoded < self.target_frames_decoded:
            ...
            msg = "At frame %d, after processing_emitting # states %d # tokens %d" % (
                self.num_frames_decoded,
                len(self.cur_toks),
                len(self.get_all_cur_toks()),
            )
            print(msg)
            # logging.info(msg)
            ...
            msg = (
                "At frame %d, after processing_nonemitting # states %d # tokens %d"
                % (
                    self.num_frames_decoded,
                    len(self.cur_toks),
                    len(self.get_all_cur_toks()),
    ...
    def reached_final(self):
        ...
        """
        for state, toks in self.cur_toks.items():
            for tok in toks:
                if tok.cost != float("inf") and self.fst.final(
                    state
                ) != fst.Weight.Zero(self.fst.weight_type()):
                    return True
        return False
    ...
    def process_emitting(self):
        ...
        if (
            ...
            for arc in self.fst.arcs(best_token.arc.nextstate):
                if arc.ilabel != 0:
                    ac_cost = self.log_likelihood_scaled[frame, int(arc.ilabel) - 1]
        ...
        for state, toks in self.prev_toks.items():
            for tok in toks:
                        ...
                        if arc.ilabel != 0:
                            ...
                            if new_weight < next_weight_cutoff:
                                ...
                                if arc.nextstate in self.cur_toks:
                                    self.cur_toks[arc.nextstate].add(new_tok)
                                else:
            ...
            for tok in toks:
                ...
                for arc in self.fst.arcs(state):
                    if arc.ilabel == 0:
                        ...
                        new_tok = Token(arc, 0.0, new_prefix, tok)
    ...
    def get_cutoff(self):
        ...
        best_cost = float("inf")
        best_token = None
        tok_count = len(self.get_all_prev_toks())

        if self.max_active == sys.maxsize and self.min_active == 0:
            for _, toks in self.prev_toks.items():
                for tok in toks:
            ...
            for state, toks in self.cur_toks.items():
                for tok in toks:
                    if tok.prefix not in prefix2cost:
                        ...
                        prefix2cost[tok.prefix] = [
                            -np.logaddexp(-prefix2cost[tok.prefix][0], -tok.cost),
                            prefix2cost[tok.prefix][1] + 1,
                        ]
    ...
    def get_all_cur_toks(self):
        all_cur_toks = set()
        for _, toks in self.cur_toks.items():
            all_cur_toks.update(toks)
        return all_cur_toks

    def get_all_prev_toks(self):
        all_prev_toks = set()
        for _, toks in self.prev_toks.items():
            all_prev_toks.update(toks)
        return all_prev_toks
