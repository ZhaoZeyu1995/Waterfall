...
class WFSTDecoder:
    ...
    def init_decoding(self):
        ...
        self.cur_toks = {}  # (prefix , state)-> token, where prefix is tuple of int
    ...
    def reached_final(self):
        ...
        for (prefix, state), tok in self.cur_toks.items():
            if tok.cost != float("inf") and self.fst.final(state) != fst.Weight.Zero(
    ...
    def process_emitting(self):
        ...
        logging.info(
            "For frame %d, there are %d tokens" % (frame, tok_count)
                        ...
                        if new_weight < next_weight_cutoff:
                            ...
                            if arc.olabel == 0:
                                if (prefix, arc.nextstate) in self.cur_toks:
                                    if (
                                        self.cur_toks[(prefix, arc.nextstate)].cost
                                        > new_tok.cost
                                    ):
                                        delete_token(
                                            self.cur_toks[(prefix, arc.nextstate)]
                                        )
                                        self.cur_toks[(prefix, arc.nextstate)] = new_tok
                                    else:
                                ...
                                if (new_prefix, arc.nextstate) in self.cur_toks:
                                    if (
                                        self.cur_toks[(new_prefix, arc.nextstate)].cost
                                        > new_tok.cost
                                    ):
                                        delete_token(
                                    ...
                                    self.cur_toks[(new_prefix, arc.nextstate)] = new_tok
                        ...
                        if arc.olabel == 0:
                            if (prefix, arc.nextstate) in self.cur_toks.keys():
                                ...
                                if (
                                    ...
                                    delete_token(self.cur_toks[(prefix, arc.nextstate)])
                                    self.cur_toks[(prefix, arc.nextstate)] = new_tok
