...
class Wav2VecFineTuning(pl.LightningModule):
    ...
    def align(self, batch):
        with torch.no_grad():
            ...
            log_gamma_norm = prior.compute_align(
                log_probs=log_probs,
                input_lengths=xlens,
                trans=trans,
    ...
    def configure_optimizers(self):
        ...
        return [optimiser, optimiser_output], [
            {
            ...
            {
                "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(
                    optimiser_output, "min", patience=2, verbose=True
                ),
                "monitor": "valid_loss",
            },
        ]
...
class Wav2VecFineTuningDiverse(pl.LightningModule):
    def __init__(self, output_size, lang_dir=None, cfg=None):
        super().__init__()
        self.output_size = output_size
        self.cfg = cfg
        self.save_hyperparameters()
        ...
        self.output_layer = nn.Linear(self.encoder_output_size, self.output_size)
        ...
        if lang_dir and self.cfg["loss"] in ["k2"]:
            self.lang = Lang(
                lang_dir, load_topo=True, load_lexicon=True, load_den_graph=True
            )
    ...
    def freese_and_init(self):
        ...
        for para in self.wav2vec.parameters():
            para.requires_grad = False
        if (
            "train_output_layer_only" in self.cfg.keys()
            and self.cfg["train_output_layer_only"]
        ):
            return
        else:
            for i in range(1, self.cfg["finetune_layers"] + 1):
    ...
    def division(self, batch, batch_idx=None, optimizer_idx=None):
        if self.cfg["loss"] == "ctc":
            ...
            if "no_den" in self.cfg.keys() and self.cfg["no_den"]:
                ...
                if (
                    "den_with_lexicon" in self.cfg.keys()
                    and self.cfg["den_with_lexicon"]
                ):
                ...
                if "mask_inf" not in self.cfg.keys() or not self.cfg["mask_inf"]:
                    denominator = graph.graphloss(
                        ...
                        dense_fsa_vec=dense_fsa_vec,
                        output_beam=self.cfg["output_beam"]
                        if "output_beam_den" not in self.cfg.keys()
                    ...
                    denominator = torch.masked_select(denominator, inf_mask).sum()
    ...
    def configure_optimizers(self):
        if (
            "same_optimiser_for_finetuning" in self.cfg.keys()
            and self.cfg["same_optimiser_for_finetuning"]
        ):
            optimiser = torch.optim.Adam(self.parameters(), lr=float(self.cfg["lr"]))
            if (
                self.cfg["loss"] == "ctc_softmax"
                and "auto_eta_scheduler" in self.cfg.keys()
                and self.cfg["auto_eta_scheduler"]
            ):
                return [optimiser], [
                    {
                        "scheduler": eta_scheduler.ReduceLROnPlateau(
                            optimiser,
                            "min",
                            patience=self.cfg["patience_eta"],
                            verbose=True,
                            factor=0.2,
                        ),
                        "monitor": "valid_loss",
