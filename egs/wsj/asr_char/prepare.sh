...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    char_local_dict=data/local/lang_char
    nlsyms=$char_local_dict/nlsyms
    if [ -d $char_local_dict ]; then
        rm -rf $char_local_dict
    fi
    mkdir -p $char_local_dict

    (cat data/train_si284/text | cut -f 2- -d" " | tr " " "\n" | sort | uniq; cat data/lang_nosp_bd/words.txt | awk '{print $1}') \
        | sort | uniq | grep -v -E '<eps>|<s>|</s>|#0|^#' > $char_local_dict/words.txt  || exit 1;
    ...
    cat $char_local_dict/words.txt | grep -E '^\{|^\}|^~|^<' >> ${nlsyms}_tmp
    cat ${nlsyms}_tmp | sort | uniq > ${nlsyms}

    word2token.py -l ${nlsyms} $char_local_dict/words.txt > $char_local_dict/lexicon.txt

    cat $char_local_dict/lexicon.txt | cut -f 2- -d" " | tr " " "\n" | sort | uniq > $char_local_dict/nonsilence_phones.txt
    echo "<space>" > $char_local_dict/silence_phones.txt
    echo "<space>" > $char_local_dict/optional_silence.txt

    utils/prepare_lang.sh --position-dependent-phones false $char_local_dict \
                        "<SPOKEN_NOISE>" ${char_local_dict}_tmp data/lang_char
    cp -r data/lang_char data/lang_char_bd
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    for topo in $topos; do
        for suffix in $lm_suffixes; do
            prepare_graph.sh --type $topo data/lang_char_${suffix} data/local/lang_char_${suffix}_${topo}_tmp
        done
    done
fi
