...
for decode_set in $decode_sets; do
    ...
    predict_dir=$exp_dir/decode_t_plus_lg_${decode_set}_${suffix}
    ...
    for beam in $beams; do 
        for maxac in $max_active; do 
            for acwt in $acoustic_scale; do
                func/decode_t_plus_lg.sh --nj $nj --max_active $maxac --acoustic_scale $acwt --beam $beam $data_dir $lang_dir $predict_dir
            done
        done
    done
done
