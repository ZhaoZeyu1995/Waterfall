...
if [ -f $srcdir/segments ]; then
  ...
  utils/apply_map.pl -f 1 $destdir/reco_map <$srcdir/wav.scp | sed 's/| *$/ |/' | \
    ...
    awk -v factor=$factor \
        '{wid=$1; $1=""; if ($NF=="|") {print wid $_ " sox -V0 -t wav - -t wav - speed " factor " |"}
          else if (match($0, /:[0-9]+$/)) {print wid " wav-copy" $_ " - | sox -V0 -t wav - -t wav - speed " factor " |" } 
          else  {print wid " sox -V0 -t wav" $_ " -t wav - speed " factor " |"}}' > $destdir/wav.scp
