...
for decode_set in $decode_sets; do
    ...
    predict_dir=$exp_dir/decode_pyctc_lex_${decode_set}_${suffix}
            ...
            for beta in $betas; do
                for beam_prune_logp in $beam_prune_logps; do
                    for token_min_logp in $token_min_logps; do 
                        func/decode_pyctc_lex.sh --nj $nj --alpha $alpha --beta $beta --beam_width $beam_width --beam_prune_logp $beam_prune_logp --token_min_logp $token_min_logp --lm $lm $data_dir $lang_dir $predict_dir
                    done
                done
            done
        done
