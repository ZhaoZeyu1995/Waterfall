...
def main(args):
    ...
    decoder  = pyctcdecode.build_ctcdecoder(labels=tokens, kenlm_model_path=args.lm, unigrams=words)
    print(len(tokens))

    logits = np.random.rand(100, len(tokens))
    exp_logits = np.exp(logits)
    log_likelihood = np.log(exp_logits / np.expand_dims(np.sum(exp_logits, axis=-1), axis=-1))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lm', help='The language model directory', type=str, default='data/local/lm/lm_tgsmall.arpa')
