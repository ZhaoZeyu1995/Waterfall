...
if [ $# != 3 ]; then
  echo "Usage: decode_t_plus_lg.sh [options] <data_dir> <lang_dir> <predict_dir>"
...
lg_graph=${lang_dir}/LG.fst
t_graph=${lang_dir}/k2/T.fst
...
mkdir -p $decode_dir
run.pl JOB=1:$nj $decode_dir/split${nj}/log/decode_t_plus_lg.JOB.log func/decode_t_plus_lg.py --max_active $maxac --acoustic_scale $acwt --beam $beam --word_symbol_table ${lang_dir}/words.txt ${t_graph} ${lg_graph} $predict_dir/split$nj/output.JOB.scp  $decode_dir/split$nj/hyp.JOB.wrd
