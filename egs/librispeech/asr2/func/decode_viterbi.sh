...
mkdir -p $decode_dir
run.pl JOB=1:$nj $decode_dir/split${nj}/log/decode_viterbi.JOB.log func/decode_viterbi.py --max-active=$maxac --acoustic-scale=$acwt --beam=$beam --word_symbol_table=${lang_dir}/words.txt ${graph} $predict_dir/split$nj/output.JOB.scp  $decode_dir/split$nj/hyp.JOB.wrd
