...
if [ $# != 3 ]; then
  echo "Usage: decode_viterbi_sumfinal.sh [options] <data_dir> <lang_dir> <predict_dir>"
...
mkdir -p $decode_dir
run.pl JOB=1:$nj $decode_dir/split${nj}/log/decode_viterbi_sumfinal.JOB.log func/decode_viterbi_sumfinal.py --max_active $maxac --acoustic_scale $acwt --beam $beam --word_symbol_table ${lang_dir}/words.txt ${graph} $predict_dir/split$nj/output.JOB.scp  $decode_dir/split$nj/hyp.JOB.wrd
