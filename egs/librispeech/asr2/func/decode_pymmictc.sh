...
# e.g. decode_pymmictc.sh data/test_dev93 data/lang exp/mmictc_LV60K_2l/version_3/decode_test_dev93
...
if [ $# != 3 ]; then
  echo "Usage: decode_pymmictc.sh [options] <data_dir> <lang_dir> <predict_dir>"
  echo "     --nj                        # default: 10, the number of jobs."
  echo "e.g.:"
  echo " $0 data/test_dev93 data/lang_tg exp/mmictc_LV60K_2l/version_3/decode_test_dev93"
...
mkdir -p $decode_dir
if [ -z $lm ]; then
    run.pl JOB=1:$nj $decode_dir/split${nj}/log/decode_pymmictc.JOB.log func/decode_pymmictc.py --alpha $alpha --beta $beta --beam_width $beam_width --beam_prune_logp ${beam_prune_logp} --token_min_logp ${token_min_logp} --word_symbol_table ${lang_dir}/words.txt $predict_dir/split$nj/output.JOB.scp  $decode_dir/split$nj/hyp.JOB.wrd $lang_dir/k2/tokens.txt 
else
    run.pl JOB=1:$nj $decode_dir/split${nj}/log/decode_pymmictc.JOB.log func/decode_pymmictc.py --alpha $alpha --beta $beta --beam_width $beam_width --beam_prune_logp ${beam_prune_logp} --token_min_logp ${token_min_logp} --lm $lm --word_symbol_table ${lang_dir}/words.txt $predict_dir/split$nj/output.JOB.scp  $decode_dir/split$nj/hyp.JOB.wrd $lang_dir/k2/tokens.txt 
fi
