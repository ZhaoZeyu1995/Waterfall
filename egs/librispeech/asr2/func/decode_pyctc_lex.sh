...
if [ $# != 3 ]; then
  echo "Usage: decode_pyctc_lex.sh [options] <data_dir> <lang_dir> <predict_dir>"
...
mkdir -p $decode_dir
run.pl JOB=1:$nj $decode_dir/split${nj}/log/decode_pyctc_lex.JOB.log func/decode_pyctc_lex.py --alpha $alpha --beta $beta --beam_width $beam_width --beam_prune_logp ${beam_prune_logp} --token_min_logp ${token_min_logp} --lm $lm $lang_dir $predict_dir/split$nj/output.JOB.scp  $decode_dir/split$nj/hyp.JOB.wrd 
