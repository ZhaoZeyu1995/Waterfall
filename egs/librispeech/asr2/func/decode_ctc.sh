...
if [ $# != 3 ]; then
  echo "Usage: decode_ctc.sh [options] <data_dir> <lang_dir> <predict_dir>"
...
mkdir -p $decode_dir
run.pl JOB=1:$nj $decode_dir/split${nj}/log/decode_ctc.JOB.log func/decode_ctc.py  $predict_dir/split$nj/output.JOB.scp  $decode_dir/split$nj/hyp.JOB.wrd
