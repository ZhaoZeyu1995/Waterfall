#!/bin/bash
#
# Prepare a phoneme dict based on the official lexicon in Librispeech which is the input for utils/prepare_lang.sh
# This programme generates two dict, for training and testing, respectively.
# data/local/dict data/local/dict_test
...
# For testing, the lexicon only contains the words in the evaluation sets, which may not be completely covered by the LMs to reduce the size of the decoding graph.
# Usage: ./local/prepare_phone_dict.sh 
# Example: ./local/prepare_phone_dict.sh
# Note this script needs inputs/lexicon.train_dev.txt and inputs/lexicon.eval.txt which are extracted from the official lexicon in Librispeech by Zeyu Zhao.
...
(echo "<UNK>"; awk '{$1=""; sub(/^ +/, ""); print $0}' inputs/lexicon.train_dev.txt | tr ' ' '\n' | sort | uniq) > $train_dict/nonsilence_phones.txt
