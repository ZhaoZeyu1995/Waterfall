...
# usage: prepare_2state.sh data/lang
...
(echo "<eps>"; echo "<blk>"; \
    cat $lang/phones.txt | grep -v "<eps>" | grep -v "<SIL>" | grep -v "#[^\d]" | awk '{print $1"_0"; print $1"_1"; print $1"_2"}'; \
...
get_token_fst_3state-skip-1.py $lang/phones.txt |\
...
get_token_fst_3state-skip-1_k2.py $lang/k2/phones.txt |\
