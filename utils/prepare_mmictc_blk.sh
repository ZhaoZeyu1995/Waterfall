...
# usage: prepare_mmictc_blk.sh data/lang

lang=$1

mkdir -p $lang/k2

(echo '<eps>' ; echo '<blk>' ; get_tokens_2state.py $lang/phones.txt | grep -v '<eps>') | awk '{print $1 " " NR-1}' > $lang/tokens_disambig.txt

cat $lang/tokens_disambig.txt | grep -v "#[^\d]" > $lang/tokens.txt
cat $lang/tokens_disambig.txt | grep "#[^\d]" | awk '{print $2}' > $lang/disambig.int

get_token_fst_mmictc_blk.py $lang/phones.txt |\
...
get_token_fst_mmictc_blk_k2.py $lang/k2/phones.txt |\
